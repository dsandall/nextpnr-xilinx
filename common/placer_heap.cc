...
class HeAPPlacer
{
  ...
  private:
    ...
    {
        // No break-count cap (owner decision, docs/126): the ripup REACH is controlled
        // by the netlist depth (freeze_gen --fuzzy-hops: 1 = the boundary regs only,
        // 2 = one level deeper) plus the friction threshold. A global count budget was
        // tried against the AES churn and removed — it is a non-physical control; if a
        // depth is too much churn for a design, lower the depth.
...
PlacerHeapCfg::PlacerHeapCfg(Context *ctx)
{
    ...
    sa_refine = ctx->setting<bool>("placerHeap/saRefine", true);
